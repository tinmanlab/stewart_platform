...
<div class="ball-row"><label for="ballDrive">Drive profile (resets)</label><select id="ballDrive"><option value="servo">24 V servo + sensors</option><option value="ideal">Ideal force comparison</option></select></div>
...
<details><summary>Drive and sensor parameters</summary>
...
<p id="ballHelp">Orange: ball · teal: target · blue square: delivered measurement · purple diamond: predicted position. Click the top view or use arrow keys. The dashed circle is a guide, not a wall. OFF keeps the plate level with its motors on. Reset ball respawns relative to the current plate; Reset all restores the reference setup. Ideal mode uses exact position and velocity. Sampled mode estimates velocity at capture time and predicts across latency. The 24 V drive uses sampled encoders and gyro/FK fusion; accelerometer values are specific force, not tilt. All hardware values are generic assumptions. This inset is not a camera.</p>
...
lab.startBall=function(options={}){
 lab.reset(S.createGeometry({payloadMass:0}));lab.setMode('ik');if(options.drive!=='ideal')A.enable(lab.sim);const preset=options.drive==='ideal'?{}:{sensor:'sampled',frequency:50,latency:.04,noise:.0007,kp:2.5,kd:2.8,maxRate:.24,commandTau:.04};B.enable(lab.sim,Object.assign(preset,options));lab.renderer.home();
...
function sync(){
 ...
 $('ballDrive').value=lab.sim.actuator?'servo':'ideal';$('ballSpeed').disabled=$('ballCurrent').disabled=!lab.sim.actuator;if(lab.sim.actuator){$('ballSpeed').value=lab.sim.actuator.parameters.velocityLimit*1000;$('ballCurrent').value=lab.sim.actuator.parameters.maxCurrent;}$('ballFrequency').value=b.settings.frequency;
...
$('ballSensor').onchange=()=>{let b=need();b.settings.sensor=$('ballSensor').value;b.sensor=B.sensorState(b.time);};
$('ballDrive').onchange=()=>lab.startBall({drive:$('ballDrive').value});
for(const [id,key,factor,min,max] of [['ballSpeed','velocityLimit',.001,20,100],['ballCurrent','maxCurrent',1,.1,5]])$(id).onchange=()=>{const a=lab.sim.actuator,v=Number($(id).value);if(a&&Number.isFinite(v)&&v>=min&&v<=max)a.parameters[key]=v*factor;else sync();};
$('ballFrequency').onchange=()=>{let b=need(),v=Number($('ballFrequency').value);if(Number.isFinite(v)&&v>=20&&v<=100){b.settings.frequency=v;b.sensor=B.sensorState(b.time);}else sync();};
...
$('ballCSV').onclick=()=>{
 if(!samples.length)return;const text='time_s,x_m,y_m,target_x_m,target_y_m,measured_x_m,measured_y_m,estimated_x_m,estimated_y_m,peak_current_A,peak_leg_speed_mps,sensor_age_s,edge_margin_m,roll_command_rad,pitch_command_rad,phase\n'+samples.map(r=>r.join(',')).join('\n');
