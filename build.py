...
def write_manifest() -> None:
    ...
    if not commit:
        try:
            ...
            commit = None  # Source ZIPs work offline without Git; do not invent a SHA.
    manifest = {
...
def build_docs() -> None:
    ...
    for path in sorted((ROOT/'docs').glob('*.md')):
        ...
        for attr in ['src','href','poster']:
            body = body.replace(attr+'="media/',attr+'="../media/')
...
if __name__ == '__main__':
    ...
    if args.site and SITE.exists():shutil.rmtree(SITE)  # Never republish stale pages/media.
    build_app()
