...
The green trail stores at most 160 ball-centre **world XYZ** samples, about 40 ms apart. Historical points do not follow later plate motion. The latest sample connects to the current ball position, including during falling and landing. The inset instead uses a separate `deckTrail`: XY computed once with the actual plate pose at each sample time. It plots those values directly, so old chart points do not shift when the plate later translates, tilts or yaws. Its axes match the deck-relative goal, ball and sensor readouts, not an absolute world-XY camera. Each history keeps at most 160 points and neither is a control input. The chart is labelled **DECK XY · CAPTURE-TIME HISTORY**.

Saved ball state tags the history with `trailFrame: "world"`. Loading an older file with untagged XY history clears only that trail: historical world heights cannot be reconstructed without the old platform poses. Ball, drive and controller states are preserved. New world and deck histories roundtrip exactly; malformed or unknown-frame history is rejected. A world-only saved file retains its 3D history but starts the missing deck history empty: historical plate poses are unavailable, so no deck-relative past is fabricated. Reset ball clears both histories. The existing GPU depth test and approximate Canvas painter fallback remain different rendering paths, not new collision models.
